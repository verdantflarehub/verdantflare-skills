...
| 代号层级                 | 格式            | 含义           | 作用与示例                                                                                                             |
| :----------------------- | :-------------- | :------------- | :--------------------------------------------------------------------------------------------------------------------- |
| **Story (故事单元)**     | -               | **业务情境**   | 由**同一角色、同一地点、同一时间**组成的有机视听单元。生成切分依据下方单元约束，Story 不等同于一次调用。                      |
| **B (Block / Beat)**     | `B01`, `B02`... | **故事段落**   | 通常 8~15 秒的叙事/情绪段落，非接口硬下限。**对齐单元目录名**（`units/B01/`）与 `unit_id`（`B01` 或 `B01-v1`）。                        |
...
| **GU (Generation Unit)** | `B01-GU01`      | **生成单元**   | 依据三同原则组合提交给模型的批次。单次 4~15s，内部多镜最多 2~3 个 Shot（1~2 个切点）。超出时切分子单元（如 `B01-GU01`、`B01-GU02`）。 |

### 生成单元组合与拆分

当前 H3 契约单次 4–15 秒；这是本项目接口限制，不推断所有视频模型的能力。Story/Block 是叙事单位，GenerationUnit 是一次生成，两者可以一对多。

- 同一角色与服装、地点、连续时间是本工作流的连续性约束；不同情境分开生成。
- 只有总时长与内部镜头数量均符合当前 H3 约束时才合并。内部多镜最多 2–3 个 Shot（1–2 个切点），以实际宿主更严格限制为准。
- 超过时长或镜头数量时拆成多个 4–15 秒的 GenerationUnit；Block 通常 8–15 秒，但可按叙事需要调整。短镜头可从获批长素材裁切，不能提交不足 4 秒的 H3 请求。
- 拆分后检查时间线覆盖、首尾锚点与连续性。已有批次批准仅对原输入版本有效。
