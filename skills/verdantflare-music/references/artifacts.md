...
完整原创及指定人声制作的输入为（只做企划、歌词或其他单阶段工作时，仅要求该阶段输入）：

1. 一句音乐灵感。
2. 一个二选一的人声来源：一个或多个原始人声音频，或者一个已批准的人声模型。

原始录音和模型必须且只能选择一种。原始录音先准备、经人工审核后用于训练并进入创作者人声库；已批准模型直接复用，不重复训练。进入对应执行阶段前必须具备：
...
`recordings.json` 记录原始人声的资产 ID、格式、授权状态和登记时间；准备批次记录输入顺序、输出 Artifact ID、报告校验值和用户审核决定；`model.json` 记录模型 ID、获批训练材料、版本、审核状态和验证音频；项目 `assets.json` 记录逻辑文件名、资产 ID、格式、时长、校验值及使用的人声模型 ID。人声模型属于创作者，可被多个项目引用；歌曲候选、分轨和母带属于单个项目。

`创作预审报告.md` 必须按五级顺序记录各级结论、证据、阻断项、警告和修改。它是当前草案的审核快照；企划、歌词或 instructions 修改后更新受影响检查及其证据，复查跨文件一致性；可保留未受影响且仍有效的结论。
