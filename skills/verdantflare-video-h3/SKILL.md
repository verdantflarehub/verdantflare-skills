...
默认 Sol-H3，明确原版请求使用宿主的非 Sol 路线；不能静默回退。业务模型类型统一为 `minimax-h3-ref2va`，服务器渠道通过独立 `route` 字段传递：A 使用 `h3-sol`，4090 实验 B 使用 `h3-sol-4090`。提交前必须确认 `model=minimax-h3-ref2va` 与实际渠道 route 的映射，不能把渠道标识写入 `model` 字段。宿主仍兼容历史别名 `minimax-h3-sol-ref2va` 和 `minimax-h3-sol-ref2va-4090`，但新请求以 `h3-sol` / `h3-sol-4090` 为规范写法。
