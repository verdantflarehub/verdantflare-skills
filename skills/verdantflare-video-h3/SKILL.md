...
默认渠道由宿主 Video MCP 的 `H3_RUNTIME_ROUTE` 配置决定；Skill 不写死 Sol 或 VDN。用户或 Generation Unit 可通过独立 `route` 字段显式选择已声明渠道，未指定时使用宿主默认值。业务模型类型统一为 `minimax-h3-ref2va`，不能把渠道标识写入 `model` 字段。提交前确认宿主能力声明与实际映射，不能静默回退。
